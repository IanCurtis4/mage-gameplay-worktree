...
func _initialize() -> void:
	...
	_check(viewport.encloses(menu.get_global_rect()) and menu.select_button.get_global_rect().size.y > 0.0 and editor_fit, "menu controls are laid out in the viewport")
	menu.queue_free()
