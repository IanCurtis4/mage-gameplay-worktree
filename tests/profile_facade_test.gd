...
func _check_create_select_and_restart() -> void:
	...
	var selected_preset := restarted.select_preset("select-preset", 3, second_id, 1)
	_check(selected_preset["ok"] and selected_preset["new_revision"] == 4 and selected_preset["profile"].character_by_id(second_id).selected_preset == 1, "preset preference is a revisioned durable menu transaction")
	var invalid_preset := restarted.select_preset("invalid-preset", 4, second_id, 2)
	...
	var third := restarted.create_character("create-cora", 5, "Cora", &"swordsman")
